...
class MaskHead(nn.Module):
    ...
    def forward(self, x: Tensor, fpns: List[Tensor], tq: Tensor):
        x = self.lay1(x) #先经过spatial decoder(实际为一个FPN结构)
        x = self.gn1(x) #[40, 256, 15, 27]

        cur_fpn = self.adapter1(fpns[0]) #backone后的res4先经过1*1卷积运算 为 [40, 256, 30, 54]
        x = cur_fpn + F.interpolate(x, size=cur_fpn.shape[-2:], mode="nearest")
        x = self.lay2(x)
        x = self.gn2(x)

        cur_fpn = self.adapter2(fpns[1]) #backone后的res3先经过1*1卷积运算 为 [40, 256, 60, 107]
        x = cur_fpn + F.interpolate(x, size=cur_fpn.shape[-2:], mode="nearest")
        x = self.lay3(x)
        x = self.gn3(x)
        x = F.relu(x)  #[40, 256, 60, 107]

        BT, C, H, W = x.shape
        L, B, N, C = tq.shape  #[3, 8, 100, 256] L表示经过了L次的trans decoder
        T = BT // B

        x = self.out_lay(x)  #[40, 256, 60, 107]
        w = self.convert_to_weight(tq).permute(1,0,2,3) #[8, 3, 100, 256]
        w = w.unsqueeze(1).repeat(1,T,1,1,1) #[8, 5, 3, 100, 256]
        #B*T*L*N == 12000 下面就是组卷积操作
        mask_logits = F.conv2d(x.view(1, BT*C, H, W), w.reshape(B*T*L*N, C, 1, 1), groups=BT) #[1, 12000, 60, 107]
