...
class IFC(nn.Module):
    ...
    def __init__(self, backbone, transformer, mask_head, num_classes, num_queries, num_frames, aux_loss=False):
        ...
        self.input_proj = nn.Conv2d(DIM_FEEDFORWARD, hidden_dim, kernel_size=1)
    ...
    # def forward(self, samples: NestedTensor, window_tokens, res_outs):
    def forward(self, window_tokens, res_outs):
        ...
        """
        # features, pos = self.forward_pre_backbone(samples, res_outs)
        out           = self.forward_post_backbone(window_tokens, res_outs) #此处与之前Mask2Former交接

        return out

    def forward_pre_backbone(self, samples: NestedTensor, res_outs):
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples, res_outs)

        return features, pos

    # def forward_post_backbone(self, features, pos, res_outs, is_train=True):
    def forward_post_backbone(self, window_tokens, res_outs, is_train=True):
        # src_proj, mask = features.decompose() #取经过backbone后的最有一层输出res5
        
        # assert mask is not None
        encoded_src = self.input_proj(res_outs['res5']) #[40, 256, 15, 27]

        # clip_hs, encoded_src = self.transformer(   # LxQxBxTxC  #此处进入 与之前Mask2Former交接
        #     src_proj, mask, self.query_embed.weight, pos, is_train
        # )
        clip_hs = self.transformer(   # LxQxBxTxC  #此处进入 与之前Mask2Former交接
            window_tokens, self.query_embed.weight, is_train
        )
        clip_hs = clip_hs.transpose(1,2)  # LxBxQxC #[3,8,100,256]

        outputs_class = self.class_embed(clip_hs) #[3,8,100,41]
        outputs_masks = self.mask_head(
            encoded_src, #[40, 256, 15, 27]
            [res_outs['res4'], res_outs['res3']],
            clip_hs
        ) # L, B, N, T, H, W [3, 8, 100, 5, 60, 107]
        out = {'pred_logits': outputs_class[-1]} #[8, 100, 41]
        out.update({'pred_masks': outputs_masks[-1]}) #[8, 100, 5, 60, 107]
