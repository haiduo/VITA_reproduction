...
class MaskHead(nn.Module):
    ...
    def forward(self, x: Tensor, tq: Tensor):
        x = self.lay1(x) # [6, 256, 120, 216] --> [6, 256, 120, 216]
        x = self.gn1(x)
        x = F.relu(x)

        BT, C, H, W = x.shape # mask_features
        L, B, N, C = tq.shape  # L表示经过了L次的trans decoder [3, 2, 100, 256]
        T = BT // B
        
        x = self.out_lay(x) #[6, 256, 120, 216]
        w = self.convert_to_weight(tq).permute(1,0,2,3) #[1, 3, 100, 256]
        w = w.unsqueeze(1).repeat(1,T,1,1,1) #[1, 6, 3, 100, 256]
        ...
        return mask_logits #[3, 1, 100, 6, 120, 216]
