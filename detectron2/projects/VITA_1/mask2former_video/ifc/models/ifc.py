...
class IFC(nn.Module):
    ...
    def __init__(self, transformer, mask_head, num_classes, num_queries, num_frames, aux_loss=False):
        ...
        hidden_dim = transformer.dim_attenion
    ...
    def forward(self, window_tokens, mask_features,is_train=True):
        # [4, 256, 120, 216] -> 
        vq = self.transformer(   # LxQxBxTxC ---> object_encoder 与 object_decoder
            window_tokens, self.query_embed.weight, is_train
        )
        vq = vq.transpose(1,2)  # LxBxQxC #[3,1,100,256]
        outputs_class = self.class_embed(vq) #[3,1,100,41]
        outputs_masks = self.mask_head(
            mask_features, # [6, 256, 120, 216]
            vq
        ) # L, B, N, T, H, W [3, 6, 100, 5, 60, 107]
        out = {'pred_logits': outputs_class[-1]} #[6, 100, 41]
        out.update({'pred_masks': outputs_masks[-1]}) #[6, 100, 5, 60, 107]
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, num_frames):
        ...
        self.register_buffer('empty_weight', empty_weight) #https://blog.csdn.net/weixin_46197934/article/details/119518497
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, log=True):
        ...
        losses = {'loss_ce_ifc': loss_ce}

        if log:
            # TODO this should probably be a separate loss, not hacked in this one here
            losses['class_error_ifc'] = 100 - accuracy(src_logits[idx], target_classes_o)[0]
        return losses
    ...
    def loss_cardinality(self, outputs, targets, indices, num_masks):
        ...
        losses = {'cardinality_error_ifc': card_err}
        return losses
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        losses = {
            "loss_mask_ifc": sigmoid_focal_loss(src_masks, target_masks, num_masks),
            "loss_dice_ifc": dice_loss(src_masks, target_masks, num_masks),
        }
        return losses
